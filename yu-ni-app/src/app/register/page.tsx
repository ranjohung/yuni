...
import { Phone, Lock, Eye, EyeOff, ArrowRight, User, Sparkles, ShieldCheck, CheckCircle, AlertTriangle } from 'lucide-react'
...
export default function RegisterPage() {
  ...
  const passwordStrength = useMemo(() => {
    ...
    if (score < 30) return { score, label: '弱', color: 'bg-red-400', textColor: 'text-red-500' }
    if (score < 60) return { score, label: '中', color: 'bg-amber-400', textColor: 'text-amber-500' }
    if (score < 80) return { score, label: '强', color: 'bg-green-400', textColor: 'text-green-500' }
    return { score: Math.min(score, 100), label: '非常强', color: 'bg-emerald-400', textColor: 'text-emerald-500' }
  }, [password])
                  ...
                  {password && (
                    <div className="mt-2 space-y-1">
                      <div className="flex gap-1">
                        <div className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${passwordStrength.score >= 20 ? passwordStrength.color : 'bg-gray-200'}`} />
                        <div className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${passwordStrength.score >= 40 ? passwordStrength.color : 'bg-gray-200'}`} />
                        <div className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${passwordStrength.score >= 60 ? passwordStrength.color : 'bg-gray-200'}`} />
                        <div className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${passwordStrength.score >= 80 ? passwordStrength.color : 'bg-gray-200'}`} />
                        <div className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${passwordStrength.score >= 100 ? passwordStrength.color : 'bg-gray-200'}`} />
                      </div>
                      <p className={`text-xs ${passwordStrength.textColor} font-medium`}>
                        密码强度：{passwordStrength.label || '请输入密码'}
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div>
                  ...
                  <div className="relative group">
                    ...
                    <input
                      ...
                      placeholder="请再次输入密码"
                      className={`w-full pl-11 pr-12 py-3.5 bg-gray-50 border-2 rounded-2xl text-gray-800 placeholder:text-gray-400
                        focus:outline-none focus:bg-white focus:ring-4 focus:ring-indigo-50
                        transition-all duration-300 ${
                  ...
                  {confirmPassword && password === confirmPassword && (
                    <p className="text-xs text-green-500 mt-1.5 ml-1 flex items-center gap-1">
